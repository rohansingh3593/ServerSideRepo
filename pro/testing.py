...
class Elder:
    ...
    def imp_word(self,active_sheet):
        from collections import Counter
        rohan1 = [active_sheet.cell(row=i, column=1).value for i in range(1,Elder.row)]

        rohan2= []
        for i in rohan1:
            if i is not None:
                if len(i.split()) > 1:
                    rohan2.extend(i.split())
                rohan2.append(i)

        sorted_word = sorted(Counter(rohan2).items(),key = lambda l: l[1],reverse=True)
        sorted_word = [i[0] for i in sorted_word ]
    ...
    def root_word(self,text):
        ...
        for i, s in enumerate(wb_obj.sheetnames):
            if s == text:
                ...
                sheet_name = wb_obj.active
                imp = self.imp_word(sheet_name)
                # print(text,s)
                break
