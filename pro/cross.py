...
count=0  
imp=0
meter_word={"MeterDb","meter","Gen5Meter",'aman',"test_fwman","sqlite",
             'rohan','remote SSH',"meterMan","repack",'agent','kurt',"package",
             "Appserv", "install_meter","firmware",'obis_code','document',"log","rahul","container"}
german_word={"start"}
pytest_word={"pytest","debug","KaizenBot","repeat"}
azure_word={"devops","developer","deploy","pipeline","work",'git','azure'}
os_word={'window','linux','command','user','vim','os','Thread','file','apache','server','python'}
rohan_word={"utils.py","shortcut"}
selenium_word={"installtion"}
vgs_word={"vgs",'COSEM'}
...
class Elder:
    ...
    def __init__(self, master):
        ...
        for i in Elder.sheet_name:
            self.Button = Button(myFrame1, text=i,command=lambda m=i: self.root_word(m))
            self.Button.configure(text=i.title())
            a = ord(i.split(" ")[-1][0].lower())%len(Elder.color)
